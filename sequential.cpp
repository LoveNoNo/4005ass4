...
void update(float *data, float *new_data) {
    ...
    for (int i = 1; i < size - 1; i++){ // 计算范围：最外圈一层是wall或者fire 温度一直保持不变，所以从1开始到size-2结束
...
void maintain_fire(float *data, bool* fire_area) {
    ...
    int len = size * size;
    for (int i = 0; i < len; i++){
        if (fire_area[i]) data[i] = fire_temp; //只要file area为1的地方就让温度始终等于炉火的温度
    }
}
...
void maintain_wall(float *data) {
    ...
    for(int i=0; i<size; i++){
        data[i] = wall_temp; // 第一行 【疑问】是否第一行会覆盖掉炉火？因为当前无法准确得知炉火是墙壁一层还是突出来（可以先maintain墙再火）
...
void master(){
    ...
    while (true) {
        ...
        if(count > 500){ //set Max iteration times
            break;
        }
